...
public class ClLogAgentApplication {
	...
	public static void main(String[] args) {
		ConfigurableApplicationContext context = SpringApplication.run(ClLogAgentApplication.class, args);
		// 采用多线程的方式，启动 收集任务，有多少个需要收集的日志，就起多少个线程
		int count = LogFileConfig.getLogFileCount();
		ExecutorService executorService = Executors.newFixedThreadPool(count);
		LogFileConfig.getConfigs().forEach(config->{
			executorService.execute(new LogTask(config));
		});
	}
